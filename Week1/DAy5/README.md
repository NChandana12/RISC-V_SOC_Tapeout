...
Welcome to **Day 5** of the RTL Workshop! Today, we cover **optimization in Verilog synthesis**, focusing on `if-else` statements, `for` loops, generate blocks, and latch inference. Hands-on labs are included.  
...
**If-else statements** allow conditional execution inside procedural blocks (`always`, `initial`, tasks, or functions).
...
--Use complete if-else and case statements to avoid latch inference

--For loops & generate blocks allow scalable and synthesizable designs

--Assign every signal in all execution paths for combinational blocks

--Labs reinforce concepts with practical Verilog code and synthesis results
